...
requireMatch(/\[["']get["'],\s*["']prepare["'],\s*["']commit["']\]\.includes\(action\)/, 'action allowlist missing');
...
requireMatch(/rejectUntrustedBrowserOrigin\(req\)/, 'untrusted browser origins must fail closed');
requireMatch(/withRestrictedCors\(req, await handle\(req\)\)/, 'successful browser responses must use restricted CORS');
