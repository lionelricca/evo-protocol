...
$('sealForm').onsubmit=async e=>{e.preventDefault();const out=$('createResult');try{
  if(!account||!walletProvider)await connectWallet();const entitlement=window.evoRefreshEntitlement?await window.evoRefreshEntitlement(account):window.evoEntitlement;if(entitlement&&!entitlement.demoAvailable&&Number(entitlement.remainingCredits||0)<1)throw new Error('Esta wallet ya usó su Passport gratuito y no tiene créditos disponibles. Comprá un crédito antes de continuar.');const file=$('file').files[0];const createdAt=new Date().toISOString(),nonce=rand(),issuerWallet=account.toLowerCase();
  ...
  toast(entitlement?.demoAvailable?'Firma tu Passport GRATIS. No es un pago ni consume gas.':'Firma tu Passport. Se usará 1 crédito; no es una transacción blockchain.');const signature=await walletProvider.request({method:'personal_sign',params:[signatureMessage,account]});const seal={sealId,version:'EVO-SEAL-V1',...metadata,metadataHash,digest,signature,signatureMessage};await registerSeal(seal);if(window.evoRefreshEntitlement)await window.evoRefreshEntitlement(account);
  out.className='result';out.innerHTML=rows({...seal,seal_id:sealId,issuer_wallet:issuerWallet,issuer_label:metadata.issuerLabel,created_at:createdAt,asset_hash:metadata.assetHash})+`<div class="actions"><button id="copyId" class="btn">Copiar Seal ID</button><a class="btn gold" href="?seal=${encodeURIComponent(sealId)}#verify">Abrir verificación pública</a></div>`;renderQrCards(out);$('copyId').onclick=()=>navigator.clipboard.writeText(sealId).then(()=>toast('Seal ID copiado'));$('verifyId').value=sealId;toast('Pasaporte EVO registrado públicamente');
...
$('verifyBtn').onclick=verify;$('clearBtn').onclick=()=>{$('sealForm').reset();$('createResult').className='empty';$('createResult').textContent='Conectá tu wallet para crear el pasaporte.'};
