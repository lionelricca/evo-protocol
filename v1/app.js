...
$('sealForm').onsubmit=async e=>{e.preventDefault();const out=$('createResult');try{
  ...
  toast(entitlement?.demoAvailable?'Firma tu Passport GRATIS. No es un pago ni consume gas.':'Firma tu Passport. Se usará 1 EVO Proof; no es una transacción blockchain.');const signature=await walletProvider.request({method:'personal_sign',params:[signatureMessage,account]});const seal={sealId,version:'EVO-SEAL-V1',...metadata,metadataHash,digest,signature,signatureMessage};await registerSeal(seal);if(window.evoRefreshEntitlement)await window.evoRefreshEntitlement(account);
