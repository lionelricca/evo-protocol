...
async function buildEvidence(db: any, sealId: string) {
  ...
  const state = {
    ...
    sealId,
    sealDigest: String(seal.digest || "").toLowerCase(),
    sealStatus: String(seal.status || "ACTIVE").toUpperCase(),
    issuerWallet,
    issuerTrust: String(profile?.status || "SELF_DECLARED").toUpperCase(),
    issuerProfileHash: String(profile?.profile_hash || "NONE").toLowerCase(),
    currentOwner,
    passportHead: String(latestEvent?.event_digest || "NONE").toLowerCase(),
    pulseHead: String(latestPulse?.pulse_hash || "NONE").toLowerCase(),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const submittedEvidence = String(p.evidenceRoot || "").toLowerCase();
    const submittedPrevious = String(p.previousContinuityRoot || "");
    const submittedRoot = String(p.continuityRoot || "").toLowerCase();
    const signer = String(p.signerWallet || "").toLowerCase();
    const signedAt = String(p.signedAt || "");
    const signature = String(p.signature || "");
    const signatureMessage = String(p.signatureMessage || "");

    if (!hex64.test(submittedEvidence) || !hex64.test(submittedRoot)) return json({ error: "invalid_root" }, 400);
    if (!(submittedPrevious === "GENESIS" || hex64.test(submittedPrevious))) return json({ error: "invalid_previous_root" }, 400);
    if (!walletRe.test(signer)) return json({ error: "invalid_signer" }, 400);
    ...
    if (age > 5 * 60 * 1000 || age < -60 * 1000) return json({ error: "stale_or_future_signature" }, 409);
    if (alreadyCheckpointed) return json({ error: "evidence_already_checkpointed", latestCheckpoint: latest }, 409);
    if (submittedEvidence !== evidence.evidenceRoot) return json({ error: "stale_evidence_root", currentEvidenceRoot: evidence.evidenceRoot }, 409);
