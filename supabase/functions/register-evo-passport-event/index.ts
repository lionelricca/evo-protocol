...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const {data,error}=await supabase.rpc("evo_register_passport_event_authoritative",{p_row:row}).single();
    if(error){
      ...
      if(message.includes("event_id_conflict"))return json({error:"event_id_conflict"},409);
      if(String(error.code||"")==="23505")return json({error:"event_already_exists"},409);
      console.error(error.code||"passport_event_rpc_error");
      return json({error:"database_error"},500);
    }
    return json({
      ok:true,
      event:{event_id:data.event_id,seal_id:data.seal_id,event_type:data.event_type,registered_at:data.registered_at,status:data.status},
      currentOwner:data.current_owner,
      authority:"OWNER_SIGNED",
      authoritative:true,
