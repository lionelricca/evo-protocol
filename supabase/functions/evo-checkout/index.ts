...
const CHAINS = {
  ...
  "10": {
    ...
    confirmations: 64n,
    rpc: ["https://optimism.drpc.org", "https://optimism-rpc.publicnode.com"],
  },
  "137": {
    name: "Polygon",
    usdc: "0x3c499c542cef5e3811e1192ce70d8cc03d5c3359",
    confirmations: 64n,
    rpc: ["https://polygon.drpc.org", "https://polygon.publicnode.com"],
  },
  "8453": {
    name: "Base",
    usdc: "0x833589fcd6edb6e08f4c7c32d4f71b54bda02913",
    confirmations: 64n,
    rpc: ["https://base.drpc.org", "https://mainnet.base.org"],
  },
  "42161": {
    name: "Arbitrum",
    usdc: "0xaf88d065e77c8cc2239327c5edb3a432268e5831",
    confirmations: 64n,
    rpc: ["https://arbitrum-one.public.blastapi.io", "https://arbitrum.api.onfinality.io/public", "https://arb1.arbitrum.io/rpc", "https://arbitrum.drpc.org"],
  },
  "43114": {
    name: "Avalanche",
...
function verifyTransfer(view: any, chain: any, txHash: string, payer: string, amount: bigint) {
  ...
  return {
    blockNumber,
    blockHash: normalize(receipt.blockHash),
    confirmations,
  ...
  try {
    ...
    try {
      ...
      for (const endpoint of chain.rpc) {
        try {
          views.push(await readChain(endpoint, chainId, txHash));
          if (views.length === 2) break;
        } catch (error) {
          console.warn("RPC fallback", chain.name, error);
        }
      }
      if (views.length < 2) throw new Error("rpc_quorum_unavailable");
    } catch (error) {
      console.error(error);
      return json({ error: "blockchain_temporarily_unavailable" }, 503);
    }
    if (views.some((view: any) => view.pending)) {
      return json({ ok: false, pending: true, reason: "transaction_not_indexed" }, 202);
    }

    let checks;
    try {
      checks = views.map((view: any) => verifyTransfer(view, chain, txHash, payer, plan.amount));
    } catch (error) {
      const reason = error instanceof Error ? error.message : "payment_verification_failed";
      return json({ error: "payment_verification_failed", reason }, 400);
    }
    if (checks[0].blockNumber !== checks[1].blockNumber || checks[0].blockHash !== checks[1].blockHash) {
      return json({ error: "rpc_consensus_failed" }, 503);
    }
    ...
    if (confirmations < chain.confirmations) {
      return json({
        ...
        requiredConfirmations: Number(chain.confirmations),
      }, 202);
    }
    ...
    if (error) {
      console.error(error);
      return json({ error: "credit_application_failed" }, 500);
    }
    return json({
      ok: true,
      pending: false,
