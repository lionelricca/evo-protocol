...
The first V4.1.1 milestone defines what EVO may expose publicly **after** a trusted server-side verifier has checked an NFC cryptographic proof.
...
The current NXP public product information describes NFC Forum Type 4 interoperability, AES-128 security, SUN/Secure Dynamic Messaging behavior and TagTamper status capabilities. EVO must not invent cryptographic field layouts or keys from memory.

## V4.1.1 implementation
...
V4.1.1 remains laboratory-only until physical tests demonstrate that:
