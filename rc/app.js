...
function qrMarkup(id){return `<div class="qrCard"><div class="qrSlot" data-seal="${esc(id)}" aria-label="QR de verificación EVO"></div><div class="qrMeta"><b>EVO Verify QR</b><p>Escaneá este código para abrir la verificación pública del registro.</p><span class="mono qrUrl">${esc(verificationUrl(id))}</span><div class="actions"><button class="btn qrCopy" type="button">Copiar enlace</button><button class="btn qrDownload" type="button">Descargar QR</button><button class="btn gold sealDownload" type="button">Descargar sello EVO</button></div></div></div>`}
function assetDetailsMarkup(s){
  const d=s?.assetDetails||s?.metadata?.assetDetails;if(!d||typeof d!=='object')return '';
  const items=[['Fabricante',d.manufacturer],['Modelo',d.model],['Año',d.manufactureYear],['Ubicación pública',d.publicLocation],['Mantenimiento',d.serviceIntervalHours?d.serviceIntervalHours+' h':'']];
  const visible=items.filter(([,v])=>String(v||'').trim());if(!visible.length)return '';
  return `<h3>Ficha técnica</h3>${visible.map(([k,v])=>`<div class="kv"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}`;
}
function rows(s,extra=''){const id=s.seal_id||s.sealId;return `<span class="status ok">✓ REGISTERED</span>${extra}<div class="kv"><span>Seal ID</span><b class="mono">${esc(id)}</b></div><div class="kv"><span>Nombre</span><b>${esc(s.title)}</b></div><div class="kv"><span>Emisor</span><span>${esc(s.issuer_label||s.issuerLabel||s.issuer_wallet||s.issuerWallet)}</span></div><div class="kv"><span>Wallet firmante</span><span class="mono">${esc(s.issuer_wallet||s.issuerWallet||'')}</span></div><div class="kv"><span>Fecha</span><span>${esc(s.created_at||s.createdAt||'')}</span></div><div class="kv"><span>Asset hash</span><span class="mono">${esc(s.asset_hash||s.assetHash||'N/A')}</span></div><div class="kv"><span>Estado</span><span class="status ok">EVO RECORD · SIGNED</span></div>${assetDetailsMarkup(s)}${qrMarkup(id)}`}
...
function makePrintableSeal(qrCanvas,id,url){
  ...
  x.fillStyle='#ffffff';x.font='900 54px Arial, sans-serif';x.fillText('EVO PROOF',220,118);
  x.fillStyle='#f4ca75';x.font='700 22px Arial, sans-serif';x.fillText('DIGITAL EVIDENCE',222,158);
...
$('walletBtn').onclick=async()=>{try{await connectWallet();toast('MetaMask conectado. Ningún pago se realiza sin tu confirmación explícita.')}catch(e){toast(e.message||'Conexión cancelada')}};
...
async function fetchSeal(id){const q=new URL(`${SUPABASE_URL}/rest/v1/evo_seals`);q.searchParams.set('seal_id',`eq.${id}`);q.searchParams.set('select','seal_id,version,asset_type,title,issuer_wallet,issuer_label,serial,description,file_name,file_size,file_type,asset_hash,metadata_hash,digest,created_at,registered_at,status,metadata');const r=await fetch(q,{headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`}});if(!r.ok)throw new Error(`No se pudo consultar el registro (${r.status})`);const result=await r.json();return result[0]||null}

$('sealForm').onsubmit=async e=>{e.preventDefault();const out=$('createResult');try{
  if(!account||!walletProvider)await connectWallet();const entitlement=window.evoRefreshEntitlement?await window.evoRefreshEntitlement(account):window.evoEntitlement;if(entitlement&&!entitlement.canCreate)throw new Error('Esta wallet ya usó su Proof gratuito y no tiene EVO Proof disponible. Comprá 1 EVO Proof antes de continuar.');const file=$('file').files[0];const createdAt=new Date().toISOString(),nonce=rand(),issuerWallet=account.toLowerCase();
  const assetDetails={manufacturer:$('assetManufacturer').value.trim(),model:$('assetModel').value.trim(),manufactureYear:$('assetYear').value.trim(),publicLocation:$('assetLocation').value.trim(),serviceIntervalHours:Number($('serviceIntervalHours').value||0)};
  const metadata={assetType:$('type').value,title:$('title').value.trim(),issuerLabel:$('issuer').value.trim(),serial:$('serial').value.trim(),description:$('description').value.trim(),fileName:file?.name||'',fileSize:file?.size||0,fileType:file?.type||'',assetHash:file?await shaFile(file):'',issuerWallet,createdAt,nonce,assetDetails};
  ...
  toast(entitlement?.demoAvailable?'Firma tu EVO Proof GRATIS. No es un pago ni consume gas.':'Firma tu EVO Proof. Se usará 1 EVO Proof; no es una transacción blockchain.');const signature=await walletProvider.request({method:'personal_sign',params:[signatureMessage,account]});const seal={sealId,version:'EVO-SEAL-V1',...metadata,metadataHash,digest,signature,signatureMessage};await registerSeal(seal);if(window.evoRefreshEntitlement)await window.evoRefreshEntitlement(account);
  out.className='result';out.innerHTML=rows({...seal,seal_id:sealId,issuer_wallet:issuerWallet,issuer_label:metadata.issuerLabel,created_at:createdAt,asset_hash:metadata.assetHash})+`<div class="actions"><button id="copyId" class="btn">Copiar Seal ID</button><a class="btn gold" href="?seal=${encodeURIComponent(sealId)}#verify">Abrir verificación pública</a></div>`;renderQrCards(out);$('copyId').onclick=()=>navigator.clipboard.writeText(sealId).then(()=>toast('Seal ID copiado'));$('verifyId').value=sealId;toast('Registro EVO publicado');
...
$('verifyBtn').onclick=verify;$('clearBtn').onclick=()=>{$('sealForm').reset();$('createResult').className='empty';$('createResult').textContent='Conectá tu wallet para crear un registro EVO.'};
...
console.info('EVO register client V4 RC',{mode:'PUBLIC REGISTRY / WALLET SIGNATURE / QR VERIFY / PRINTABLE PROOF / NO TOKEN MOVEMENT'});